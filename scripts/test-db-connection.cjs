...
async function testDatabaseConnection() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   Database: ${result.rows[0].db_version.split(' ')[0]} ${result.rows[0].db_version.split(' ')[1]}`
      );
      ...
      if (tablesResult.rows.length > 0) {
        ...
        console.log(
          '⚠️  No tables found. You may need to run the schema setup.'
        );
      }
      ...
      try {
        ...
        const achievementsCount = await client.query(
        ...
        console.log(
          '⚠️  Some tables may not exist yet. Run the schema setup first.'
        );
      }
    } finally {
      client.release();
...
console.log(
  `   Database Host: ${process.env.POSTGRES_HOST || process.env.DB_HOST || 'not set'}`
);
console.log(
  `   Database Name: ${process.env.POSTGRES_DATABASE || process.env.DB_NAME || 'not set'}`
);
